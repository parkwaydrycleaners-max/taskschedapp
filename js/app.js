    ...
    for (let dayOffset = 0; dayOffset < this.viewDays; dayOffset++) {
        ...
        whiteboard.appendChild(dayContainer);
            ...
            createDayContainer(date, dayOffset) {
                ...
                // Determine if this date is today, past, or future
                const today = new Date();
                today.setHours(0, 0, 0, 0);
                ...
                // Always show prominent day header (even for single day view)
                ...
                // Dynamic styling based on date relationship to today
                let headerClasses = 'text-center py-3 px-4 rounded-t-xl mb-3 font-bold text-lg shadow-sm border-b-4 ';
                let dateIndicator = '';

                if (isToday) {
                    headerClasses += 'bg-gradient-to-r from-blue-500 to-indigo-600 text-white border-blue-700';
                    dateIndicator = ' <span class="ml-2 px-2 py-0.5 bg-white/20 rounded-full text-sm font-medium">TODAY</span>';
                } else if (isPast) {
                    headerClasses += 'bg-gradient-to-r from-gray-400 to-gray-500 text-white border-gray-600 opacity-80';
                    dateIndicator = ' <span class="ml-2 px-2 py-0.5 bg-white/20 rounded-full text-sm font-medium">PAST</span>';
                } else {
                    headerClasses += 'bg-gradient-to-r from-emerald-500 to-teal-600 text-white border-emerald-700';
                    // Show how many days in future
                    const daysAhead = Math.ceil((compareDate - today) / (1000 * 60 * 60 * 24));
                    if (daysAhead === 1) {
                        dateIndicator = ' <span class="ml-2 px-2 py-0.5 bg-white/20 rounded-full text-sm font-medium">TOMORROW</span>';
                    } else {
                        dateIndicator = ` <span class="ml-2 px-2 py-0.5 bg-white/20 rounded-full text-sm font-medium">+${daysAhead} DAYS</span>`;
                    }
                }

                dayHeader.className = headerClasses;
                dayHeader.innerHTML = `
                    <div class="flex items-center justify-center">
                        <span class="text-xl font-extrabold tracking-wide">${dayName.toUpperCase()}</span>
                        ${dateIndicator}
                    </div>
                    <div class="text-sm font-medium opacity-90 mt-1">${dayStr}</div>
                `;
                ...
                this.people.forEach(person => {
                    const column = this.createPersonColumn(person, date, dayOffset, isToday, isPast);
                    columnsContainer.appendChild(column);
                });

                container.appendChild(columnsContainer);

                // Add visual separator between days when viewing multiple days
                if (this.viewDays > 1) {
                    container.className = 'flex-shrink-0 mr-6 pb-4 border-r-4 border-dashed border-gray-300 dark:border-gray-600 pr-6';
                }

                return container;
            }
            
            createPersonColumn(person, date, dayOffset, isToday = false, isPast = false) {
                const column = document.createElement('div');

                // Add subtle background tint based on day type
                let columnBgClass = 'bg-gray-50 dark:bg-gray-800';
                let borderClass = 'border border-gray-200 dark:border-gray-700';
                if (isToday) {
                    columnBgClass = 'bg-blue-50 dark:bg-blue-900/20';
                    borderClass = 'border-2 border-blue-300 dark:border-blue-700';
                } else if (isPast) {
                    columnBgClass = 'bg-gray-100 dark:bg-gray-800/50';
                    borderClass = 'border border-gray-300 dark:border-gray-600';
                }

                column.className = `flex-shrink-0 person-column w-[268px] ${columnBgClass} rounded-lg ${borderClass}`;
                column.dataset.person = person;

                const timeSlots = [];
                for (let i = 0; i < 32; i++) {
                    timeSlots.push(`<div class="time-slot relative border-gray-200 dark:border-gray-700"></div>`);
                }
                ...
                const usedSlots = personTasks.reduce((total, task) => total + task.duration, 0);
                const remainingSlots = capacity - usedSlots;
                const remainingHours = Math.floor(remainingSlots * 15 / 60);
                const remainingMins = (remainingSlots * 15) % 60;

                // Date badge styling based on day type
                let dateBadgeClass = 'bg-gray-200 text-gray-700 dark:bg-gray-700 dark:text-gray-300';
                if (isToday) {
                    dateBadgeClass = 'bg-blue-500 text-white';
                } else if (isPast) {
                    dateBadgeClass = 'bg-gray-400 text-white dark:bg-gray-600';
                } else {
                    dateBadgeClass = 'bg-emerald-500 text-white';
                }
                    ...
                    <div class="p-4 border-b border-gray-200 dark:border-gray-700">
                        <div class="flex items-center justify-between mb-2">
                            <div class="flex items-center gap-2">
                                <h3 class="font-semibold text-lg dark:text-white">${person}</h3>
                                <span class="px-2 py-0.5 rounded-full text-xs font-bold ${dateBadgeClass}">${shortDate}</span>
                            </div>
                            <div class="flex space-x-1">
                                ...
                                <button class="quick-capacity-header-btn p-1 hover:bg-purple-100 dark:hover:bg-purple-900/30 rounded text-purple-500 transition-colors" title="Quick Capacity Override" data-person="${person}" data-date="${this.dateToLocalDateString(date)}">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                                    </svg>
                                </button>
                                <button class="snap-cards-btn p-1 hover:bg-green-100 dark:hover:bg-green-900/30 rounded text-green-500 transition-colors" title="Snap Cards to Top" data-person="${person}" data-date="${this.dateToLocalDateString(date)}">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <div class="text-xs text-gray-500 dark:text-gray-400 mb-1">
                            <button class="quick-capacity-btn hover:bg-gray-200 dark:hover:bg-gray-700 px-2 py-1 rounded transition-colors text-left w-full"
                        ...
                        <div class="flex items-center justify-between text-xs">
                            ...
                            <div class="font-medium ${remainingSlots > 0 ? 'text-green-600 dark:text-green-400' : remainingSlots === 0 ? 'text-orange-500' : 'text-red-500'}">
...
if (savedConfig) {
    ...
    try {
        ...
        // If service interruption modal is already showing, don't double up
        if (!this.serviceInterruption.modalVisible) {
            // Check if this looks like a service issue
            const isServiceIssue = error.isServerError ||
                !navigator.onLine ||
                (error.message && (
            ...
            if (isServiceIssue) {
                this.serviceInterruption.isActive = true;
                this.serviceInterruption.consecutiveFailures++;
                this.showServiceInterruptionModal(error);
            }
        }
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
        try {
            ...
            if (!response.ok) {
                ...
                if (!silent) {
                    console.error('❌ Airtable error response:', statusCode, errorBody);
                }
                const errorMsg = errorBody.error?.message || `HTTP ${statusCode}`;
                const err = new Error(errorMsg);
                err.statusCode = statusCode;
                err.isServerError = statusCode >= 500;
                throw err;
            }

            const result = await response.json();

            // Reset error count on success and clear service interruption
            this.errorCount = 0;
            if (this.serviceInterruption.consecutiveFailures > 0) {
                this.serviceInterruption.consecutiveFailures = 0;
                this.serviceInterruption.isActive = false;
                if (this.serviceInterruption.modalVisible) {
                    this.hideServiceInterruptionModal();
                    this.showToast('✅ Airtable connection restored!', 'success');
                }
            }
            return result;
            ...
            if (attempt === maxRetries) {
                // Track consecutive failures for service interruption detection
                this.serviceInterruption.consecutiveFailures++;
                this.serviceInterruption.lastError = error.message;
                this.serviceInterruption.lastStatusCode = error.statusCode || null;
                this.serviceInterruption.lastAttemptTime = new Date();

                // Detect service interruption: server errors (5xx), network failures, or repeated failures
                const isServiceIssue = error.isServerError ||
                    !navigator.onLine ||
                    error.message.includes('Failed to fetch') ||
                    error.message.includes('NetworkError') ||
                    ...
                    this.serviceInterruption.consecutiveFailures >= 2;

                if (isServiceIssue) {
                    this.serviceInterruption.isActive = true;
                    this.showServiceInterruptionModal(error);
                    throw error; // Still throw so callers know it failed
                }
...
async loadAllDataFromAirtable() {
    ...
    try {
        ...
        // Only show toast if service interruption modal isn't handling it
        if (!this.serviceInterruption.isActive && !this.serviceInterruption.modalVisible) {
...
handleProductionError(error, context = '') {
    ...
    // If a service interruption is active, suppress individual toasts to avoid spam
    if (this.serviceInterruption.isActive || this.serviceInterruption.modalVisible) {
        return;
    }
...
showServiceInterruptionModal(error) {
    const modal = document.getElementById('serviceInterruptionModal');
    if (!modal) return;

    // CRITICAL: Force page visible so the modal can actually be seen.
    // In the consolidated single-file version, the FOUC-prevention
    // 'html { visibility: hidden }' may not have been cleared yet.
    document.documentElement.style.visibility = 'visible';

    // Update error details
    const errorMsgEl = document.getElementById('serviceErrorMessage');
    const errorCodeEl = document.getElementById('serviceErrorCode');
    const failCountEl = document.getElementById('serviceFailCount');
    const lastAttemptEl = document.getElementById('serviceLastAttemptTime');

    if (errorMsgEl) {
        const friendlyMsg = this.getServiceErrorDescription(error);
    ...
    if (errorCodeEl) {
        ...
        let codeText = '';
        if (statusCode) codeText += `HTTP ${statusCode}`;
        if (rawMsg && rawMsg !== codeText) codeText += codeText ? ` — ${rawMsg}` : rawMsg;
        errorCodeEl.textContent = codeText;
    }

    if (failCountEl) {
        failCountEl.textContent = this.serviceInterruption.consecutiveFailures;
    }

    if (lastAttemptEl) {
        const now = new Date();
        lastAttemptEl.textContent = now.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', second: '2-digit' });
    }

    // Show modal (only if not already visible to avoid duplicating listeners)
    if (!this.serviceInterruption.modalVisible) {
        modal.classList.remove('hidden');
        this.serviceInterruption.modalVisible = true;

        // Attach event listeners (once)
        this.attachServiceInterruptionListeners();

        // Start auto-retry countdown
        this.startServiceRetryCountdown();
    } else {
        // Modal already visible, just update the info and restart countdown
        this.startServiceRetryCountdown();
    }

    // Suppress the loading overlay if it's shown
    this.hideLoading();
}

hideServiceInterruptionModal() {
    const modal = document.getElementById('serviceInterruptionModal');
    if (modal) {
        modal.classList.add('hidden');
    }
    this.serviceInterruption.modalVisible = false;
    this.stopServiceRetryCountdown();
}

getServiceErrorDescription(error) {
    ...
    if (code === 500) {
        return 'Airtable is experiencing internal server errors. This is a service-side issue.';
    }
    if (code === 502) {
        return 'Airtable returned a bad gateway error. The service may be overloaded or down.';
    }
    if (code === 503) {
        return 'Airtable service is temporarily unavailable. This usually indicates planned or unplanned maintenance.';
    }
    if (code === 504) {
        return 'Airtable gateway timed out. The service is likely under heavy load.';
    }
    if (code === 429) {
        return 'Too many requests to Airtable. The API rate limit has been reached.';
    }
    if (msg.includes('failed to fetch') || msg.includes('networkerror') || msg.includes('load failed')) {
        return 'Unable to reach Airtable servers. This could be a network issue or an Airtable outage.';
    }
    if (code >= 500) {
        return 'Airtable is returning server errors. The service appears to be experiencing issues.';
    }
    return 'Connection to Airtable failed. The service may be temporarily unavailable.';
}

attachServiceInterruptionListeners() {
    // Remove old listeners by cloning elements
    const retryNowBtn = document.getElementById('serviceRetryNowBtn');
    const retryMainBtn = document.getElementById('serviceRetryMainBtn');
    const dismissBtn = document.getElementById('serviceDismissBtn');

    if (retryNowBtn) {
        const newRetryNow = retryNowBtn.cloneNode(true);
        retryNowBtn.parentNode.replaceChild(newRetryNow, retryNowBtn);
        newRetryNow.addEventListener('click', () => this.retryAirtableConnection());
    }

    if (retryMainBtn) {
        const newRetryMain = retryMainBtn.cloneNode(true);
        retryMainBtn.parentNode.replaceChild(newRetryMain, retryMainBtn);
        newRetryMain.addEventListener('click', () => this.retryAirtableConnection());
    }

    if (dismissBtn) {
        const newDismiss = dismissBtn.cloneNode(true);
        dismissBtn.parentNode.replaceChild(newDismiss, dismissBtn);
        newDismiss.addEventListener('click', () => this.hideServiceInterruptionModal());
    }
}

async retryAirtableConnection() {
    this.stopServiceRetryCountdown();

    // Update UI to show retrying
    const retryMainBtn = document.getElementById('serviceRetryMainBtn');
    if (retryMainBtn) {
        retryMainBtn.disabled = true;
        retryMainBtn.innerHTML = '<div class="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin"></div><span>Retrying...</span>';
    }
    const retryNowBtn = document.getElementById('serviceRetryNowBtn');
    if (retryNowBtn) {
        retryNowBtn.disabled = true;
        retryNowBtn.textContent = 'Retrying...';
    }

    try {
        // Try a lightweight request to test connectivity
        const tableName = encodeURIComponent(this.airtableConfig.tablesConfig.people || 'People');
        const testUrl = `https://api.airtable.com/v0/${this.airtableConfig.baseId}/${tableName}?maxRecords=1`;
        ...
        if (response.ok) {
            ...
            this.serviceInterruption.isActive = false;
            this.hideServiceInterruptionModal();
            this.showToast('✅ Airtable connection restored! Reloading data...', 'success');

            // Reload all data
            try {
                this.showLoading('Reloading data from Airtable...');
                await this.loadAllDataFromAirtable();
                this.renderWhiteboard();
            } catch (reloadError) {
                console.warn('Data reload after reconnect failed:', reloadError);
            } finally {
                this.hideLoading();
            }
        } else {
            ...
            err.statusCode = statusCode;
            this.serviceInterruption.lastError = err.message;
            this.serviceInterruption.lastStatusCode = statusCode;

            // Update modal with new info
            this.updateServiceInterruptionModalInfo(err);
            this.startServiceRetryCountdown();
        }
    } catch (fetchError) {
        ...
        this.serviceInterruption.consecutiveFailures++;
        this.serviceInterruption.lastAttemptTime = new Date();
        this.serviceInterruption.lastError = fetchError.message;

        this.updateServiceInterruptionModalInfo(fetchError);
        this.startServiceRetryCountdown();
    } finally {
        // Restore button states
        if (retryMainBtn) {
            retryMainBtn.disabled = false;
            retryMainBtn.innerHTML = '<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"></path></svg><span>Retry Connection</span>';
        }
        if (retryNowBtn) {
...
updateServiceInterruptionModalInfo(error) {
    const errorMsgEl = document.getElementById('serviceErrorMessage');
    const errorCodeEl = document.getElementById('serviceErrorCode');
    const failCountEl = document.getElementById('serviceFailCount');
    const lastAttemptEl = document.getElementById('serviceLastAttemptTime');
    ...
    if (errorMsgEl) {
        errorMsgEl.textContent = this.getServiceErrorDescription(error);
    }
    if (errorCodeEl) {
        const statusCode = error.statusCode || '';
        const rawMsg = error.message || '';
    ...
    if (lastAttemptEl) {
        lastAttemptEl.textContent = new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', second: '2-digit' });
    }
}

startServiceRetryCountdown() {
    this.stopServiceRetryCountdown();

    // Increase countdown interval based on consecutive failures (30s, 45s, 60s, etc.)
    const baseSeconds = 30;
    const extraSeconds = Math.min(this.serviceInterruption.consecutiveFailures * 15, 90);
    this.serviceInterruption.retryCountdownSeconds = baseSeconds + extraSeconds;

    const countdownEl = document.getElementById('serviceRetryCountdown');
    const retrySection = document.getElementById('serviceAutoRetrySection');

    if (retrySection) retrySection.classList.remove('hidden');

    let remaining = this.serviceInterruption.retryCountdownSeconds;
    if (countdownEl) countdownEl.textContent = remaining;

    this.serviceInterruption.retryCountdownTimer = setInterval(() => {
        remaining--;
        if (countdownEl) countdownEl.textContent = remaining;

        if (remaining <= 0) {
            this.retryAirtableConnection();
        }
    }, 1000);
}

stopServiceRetryCountdown() {
    if (this.serviceInterruption.retryCountdownTimer) {
        clearInterval(this.serviceInterruption.retryCountdownTimer);
        this.serviceInterruption.retryCountdownTimer = null;
    }
}
...
initOfflineDetection() {
    window.addEventListener('online', () => {
        // If service interruption modal is showing, try to reconnect
        if (this.serviceInterruption.modalVisible) {
            this.retryAirtableConnection();
        } else {
    ...
    window.addEventListener('offline', () => {
        ...
        this.showServiceInterruptionModal(offlineError);
    });
}
